...
class SliderBloc extends Bloc<SliderEvents, SliderStates> {
  ...
  Future<void> _getData(GetSliderEvent event, Emitter<SliderStates> emit) async {
    ...
    if (response.isSuccess) {
      ...
      emit(GetSliderSuccessState(list: list, currentIndex: 0));
    } else {
  ...
  void changeCurrentIndex(int index) {
    if (state is GetSliderSuccessState) {
      final currentState = state as GetSliderSuccessState;
      emit(GetSliderSuccessState(list: currentState.list, currentIndex: index));
    }
  }
}
